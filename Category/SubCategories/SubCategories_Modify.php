...
<head>
    ...
    if (!empty($_GET["id"])) {
        ...
        $Query_Select = " SELECT SubCategories.SubCategory_ID, SubCategories.Category_ID, SubCategories.SubCategory_Name, 
                        SubCategories.SubCategory_Desc, Categories.Category_Name 
                        FROM SubCategories 
                        JOIN Categories 
                        ON SubCategories.Category_ID = Categories.Category_ID 
                        WHERE SubCategories.SubCategory_ID = :SubCategory_ID";

        $pdoQuery_Select = $connexion->prepare($Query_Select);
        $pdoQuery_Select->execute(['SubCategory_ID' => $SubCategory_ID]);
        $Single_SubCategory = $pdoQuery_Select->fetch(PDO::FETCH_ASSOC);
    } else {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (!empty($_POST['SubCategory_Name'])) {
            ...
            if (!$Existing_SubCategory) {
                $Update_SubCategory = " UPDATE SubCategories 
                                        SET SubCategory_Name = :SubCategory_Name , Category_ID = :Category_ID , SubCategory_Desc = :SubCategory_Desc 
                                        WHERE SubCategory_ID = :SubCategory_ID";
                $pdoUpdate_SubCategory = $connexion->prepare($Update_SubCategory);
                $pdoUpdate_SubCategory->execute([
                ...
                $_SESSION['SubCategory_Add/Update'] = "SubCategory Updated Successfully";
    ...
    <div class="container mx-auto p-6">
        <div class="content-wrapper">
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                <div class="mb-4">
                    <label for="SubCategory_ID" class="block text-sm font-medium text-gray-700">SubCategory
                        ID:</label>
                    <span class=""><b><?php echo $Single_SubCategory['SubCategory_ID'] ?></b></span>
                </div>
                <div class="mb-4">
                    <label for="SubCategory_Name" class="block text-sm font-medium text-gray-700">SubCategory
                        Name:</label>
                    <input type="text" name="SubCategory_Name"
                        value="<?php echo $Single_SubCategory["SubCategory_Name"]; ?>"
                ...
                <div class="mb-4">
                    <select name="Category_Name" class="w-full p-2 border border-gray-300 rounded" required>
                        ...
                        foreach ($Categories_Options as $Cat) {
                            $selected = '';
                            if ($Cat === $Single_SubCategory['Category_Name']) {
                                $selected = 'selected';
                            }
                            ?>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="SubCategory_Desc" id="SubCategory_Desc" cols="30" rows="10"
                        class="mt-1 p-2 w-full border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                        placeholder="Write a few lines describing the manufacturer"><?php echo $Single_SubCategory['SubCategory_Desc']; ?></textarea>
                </div>
                <div class="mb-4">
                    <button type="submit"
