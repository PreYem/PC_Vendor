...
<head>
    ...
    if (isset($_GET["id"])) {
        $Category_ID = $_GET["id"];
        $CurrentCategory = "SELECT Category_ID , Category_Name, Category_Desc FROM Categories WHERE Category_ID = :Category_ID";
        $pdostmt = $connexion->prepare($CurrentCategory);
        $pdostmt->execute(["Category_ID" => $Category_ID]);
        $Category_Single = $pdostmt->fetch(PDO::FETCH_ASSOC);
    ...
    if (!empty($_POST)) {
        ...
        $queryCheck = "SELECT Category_ID FROM Categories
                       WHERE LOWER(Category_Name) = :Category_Name_Lower AND Category_ID != :Category_ID";
        $pdostmtCheck = $connexion->prepare($queryCheck);
        $pdostmtCheck->execute([
            "Category_Name_Lower" => $Category_Name_Lower,
        ...
        if (!$existingCategory) {
            $queryUpdate = "UPDATE Categories SET Category_Name = :Category_Name, Category_Desc = :Category_Desc 
            ...
            $_SESSION['Category_Add/Update'] = "Category Updated Successfully";
            header("Location: Categories_List.php");
            exit();
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                <div class="mb-4">
                    <label for="Category_ID" class="block text-sm font-medium text-gray-700">Category
                        ID:</label>
                    <span class=""><b><?php echo $Category_Single["Category_ID"]; ?></b></span>
                </div>
                <div class="mb-4">
                    <label for="Category_Name" class="block text-sm font-medium text-gray-700">Category
                        Name:</label>
                    <input type="text" name="Category_Name" value="<?php echo $Category_Single["Category_Name"]; ?>"
                ...
                <div class="mb-4">
                    <label for="Category_Desc" class="block text-sm font-medium text-gray-700">Category
                        Description:</label>
                    <textarea name="Category_Desc" id="Category_Desc" cols="30" rows="10"
                        class="mt-1 p-2 w-full border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                        placeholder="Write a few lines describing the manufacturer"><?php echo $Category_Single["Category_Desc"]; ?></textarea>
                </div>
                <div class="mb-4">
                    <button type="submit"
