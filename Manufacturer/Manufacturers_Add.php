...
<head>
    ...
    <title>Add Manufacturer | PC Vendor</title>
    <?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (!empty($_POST["Manufacturer_Name"])) {
            ...
            $Check_Manufacturer_Unique = "SELECT Manufacturer_ID FROM Manufacturers WHERE LOWER(Manufacturer_Name) = :Manufacturer_Name_Lower";
            $pdoCheck_Manufacturer_Unique = $connexion->prepare($Check_Manufacturer_Unique);
            $pdoCheck_Manufacturer_Unique->execute(['Manufacturer_Name_Lower' => $Manufacturer_Name_Lower]);
            $Check_Manufacturer_Unique = $pdoCheck_Manufacturer_Unique->fetchColumn();

            if (!$Check_Manufacturer_Unique) {
                $Insert_Manufacturer = "INSERT INTO Manufacturers (Manufacturer_Name, Manufacturer_Desc) VALUES (:Manufacturer_Name, :Manufacturer_Desc)";
                $pdoInsert_Manufacturer = $connexion->prepare($Insert_Manufacturer);
                $pdoInsert_Manufacturer->execute([
                    'Manufacturer_Name' => $Manufacturer_Name,
                    'Manufacturer_Desc' => $Manufacturer_Desc
                ]);

                $_SESSION['Manufacturer_Add/Update'] = "Manufacturer Added Successfully";
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <a href="Manufacturers_List.php"
                        class="ml-4 px-4 py-2 bg-gray-400 text-white rounded-md hover:bg-gray-600">X</a>
                </div>
